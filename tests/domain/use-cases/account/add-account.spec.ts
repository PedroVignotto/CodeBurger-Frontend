...
describe('AddAccountUseCase', () => {
  ...
  it('Should call HttpClient with correct values', async () => {
    await sut({ name, email, password, passwordConfirmation })

    expect(httpClient.request).toHaveBeenCalledWith({ url, method: 'post', body: { name, email, password, passwordConfirmation } })
  ...
  it('Should throw FieldInUseError if HttpClient returns 400', async () => {
    ...
    const promise = sut({ name, email, password, passwordConfirmation })
  ...
  it('Should return an account if HttpClient returns 201', async () => {
    const result = await sut({ name, email, password, passwordConfirmation })
